...
실제 지도 데이터로 만든 귀여운 로우폴리 3D 월드를 치비 캐릭터로 걸어다니며 탐험하는 웹 앱이에요.

검색한 좌표 주변 500m의 OpenStreetMap 데이터(건물 폴리곤·도로·POI)를 받아
Three.js 씬으로 압출(extrude)해서, 물리 기반 캐릭터 컨트롤러로 그 위를 돌아다니는 거예요.
...
기본 진입 위치는 **광화문 (37.576, 126.977)**, 로딩 범위는 반경 500m다.
...
지도 프로젝트에서 버그의 대부분은 좌표계 혼동에서 나와요. 그래서 세 좌표계를
**타입으로 분리**하고, 변환은 `utils/coordinates.ts` 한 곳에서만 하도록 했어요.
...
Overpass/지오코딩 응답은 스키마가 느슨하고 자주 비어요. `lib/schemas.ts`에서
파싱 단계에 검증하면, 씬 레이어에는 항상 정상 형태만 들어가도록 해요.

### 3. 렌더 비용 관리

건물은 개별 메시가 아니라 압출 후 재질을 `materialPool`에서 공유해요.
모델은 `scripts/convert-models.js`로 Draco/meshopt 압축을 거쳐 로드하고,
`PerformanceMonitor`로 프레임 예산을 눈으로 확인하면서 튜닝하는 식이에요.
...
**Source-available — 오픈소스는 아니에요.** 코드를 읽을 수 있게 공개했을 뿐,
사용 권한을 준 것은 아니에요. 다른 프로젝트에 가져다 쓰거나 재배포·상업적 이용을
